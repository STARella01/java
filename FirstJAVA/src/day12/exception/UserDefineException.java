...
public class UserDefineException {
	...
	public void insert(Member member) throws DuplicateKeyException{
		...
		// DB에 홍길동 010 1223 3847 정보가 있다고 간주
		if("홍길동".equals(member.name) && "010-1223-3847".equals(member.name) ) {
			// 이 경우에는 DB에 이미 정보가 있는 경우로 간주!
			// 예외를 발생시켜 insert 메서드 호출한 곳으로 데이터가 중복됨을 알려줘야 함.
			
			// 예외 강제로 발생!
			throw new DuplicateKeyException("데이터가 중복된다.");
		}else {
...
// 멤버 클래스 (DATA를 저장하는 용도의 객체 생성을 위한 클래스)
class Member{
	String name;
	String phone;
	String address;
