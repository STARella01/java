...
public class Pen {
	...
	// ** 기본 생성자는 매개변수를 가지지 않는 형태로, 만약 자동으로 만들어지는 경우 내용은 없다.
	// 
	/*		** 생성자를 만드는 규칙
	 * 		1. 생성자를 만들 때, 생성자의 이름은 클래스 이름과 대소문자까지 정확히 같아야 함.
	 * 		   클래스 이름과 다르게 생성하면 그것은 메서드 취급됨
	 * 		2. 생성자는 메서드와 달리 반환유형을 적지 않는다.
	 *      3. 생성자는 다른 유형으로 생성 가능 (생성자 Over loading)
	 *          
	 */
	
	
	// 생성자 오버로딩 - 생성자 여러개 불러온 경우.
	...
	public Pen(String color, int length, int price) {
		// super(); // 상속받을 부모 클래스 의미 - 현재는 미사용하므로, 주석처리
		this.color = color;
		this.length = length;
		this.price = price;
	}
