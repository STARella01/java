...
public class StreamMatch {
	...
	public static void main(String[] args) {
		...
		// allMatch( Predicate p )			: 요소들이 모두 조건 p를 만족하면 true, 그렇지 않으면 false.
		// anyMatch( Predicate p )	 	: 요소들 중 하나라도 p조건을 만족하면 true , 그렇지 않으면 false
		// noneMatch( Predicate p )		: 요소들 모두 p조건을 만족하지 않으묜 true, 그렇지 않으면 false
		...
		boolean result = list.stream().allMatch(a -> (a instanceof Shape));   // a가 Shape에 속해있느냐?
